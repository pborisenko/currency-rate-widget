/**
 * Currency Rate Widget предоставляет данные курсов валют Центрального Банка России на сайте
 */
let CRWidget = {
        ...
        try {
            ...
            if (document.getElementById(this.options['container'])) {
                ...
                for (key in this.content['valute']) {
                    ...
                    if (this.options['favorite'].includes(key)) {
    ...
    request: async function() {
        try {
            ...
            /**
             * Проверяем хранилище на актуальность данных,
             * при необходимости подгружаем новые данные и обновляем хранилище
             */
            if (!this.isActualData(this.options['id'])) {
                let response = await fetch(this._urlAPI);
                localStorage.setItem(this.options['id'], JSON.stringify( await response.json()));
            }
            
            /**
             * Загружаем данные в объект
             */
            let data = JSON.parse(localStorage.getItem(this.options['id']));
            if (data) {
    ...
    */
    ...
    formatValuteCode: function(valuteKey) {
        let str;
        let valute = this.content['valute'][valuteKey];
     ...
     */
    setElement: function(id, containerId, className=id, content='') {
    ...
    insertElement: function(element) {
        let container = document.getElementById(element.getAttribute('container'));
